...
Any client that implements the MCP specification can connect to the Lattice server. Point it at the `@lattice/mcp-server` package using `npx` as the transport command. Consult your client's documentation for the exact configuration format.
...
- All data is included in the npm package. No external data fetching is required for any tool except `oracle`.
